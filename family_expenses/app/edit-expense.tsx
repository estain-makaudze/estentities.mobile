...
/**
 * Edit Expense screen.
 * Accepts `id` as a route param (e.g. router.push({ pathname: "/edit-expense", params: { id } }))
 */
export default function EditExpenseScreen() {
  ...
  // Derive initial split mode from the existing expense.
  function resolveInitialSplitMode(): SplitMode {
    if (!expense?.splits) return "none";
    if (users.length > 0) {
      const equalPct = 100 / users.length;
      const isEqual = expense.splits.every(
        (s) => Math.abs(s.percentage - equalPct) < 0.5
      );
  ...
  const [category, setCategory] = useState(expense?.category ?? categoryNames[0] ?? "Other");
  const [note, setNote] = useState(expense?.note ?? "");
  const [splitMode, setSplitMode] = useState<SplitMode>(resolveInitialSplitMode);

  const [customPercentages, setCustomPercentages] = useState<Record<string, string>>(() => {
    if (expense?.splits && expense.splits.length > 0) {
      const percs: Record<string, string> = {};
      expense.splits.forEach((s) => {
        percs[s.userId] = String(s.percentage);
      });
      return percs;
    }
    return extractCustomPercentages(undefined, users);
  });
  ...
  if (users.length === 0) {
    return (
      <View style={styles.center}>
        ...
        >
          <Text style={styles.backBtnText}>Go to Household</Text>
        </TouchableOpacity>
      </View>
    );
  }
      ...
      >
        ...
        <View style={styles.pillRow}>
          {users.map((u) => (
            <TouchableOpacity
              key={u.id}
              style={[styles.pill, paidBy === u.id && { backgroundColor: u.color }]}
              onPress={() => setPaidBy(u.id)}
            >
              <Text style={[styles.pillText, paidBy === u.id && styles.pillTextActive]}>
                {u.name}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <View style={styles.splitModeRow}>
          {(["none", "equal", "custom"] as SplitMode[]).map((mode) => (
            ...
            >
              ...
              >
                {mode === "none" ? "No Split" : mode === "equal" ? "Equal Split" : "Custom %"}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        {splitMode === "none" && (
          <View style={styles.infoBox}>
            <Text style={styles.infoText}>
              No split — the payer covers this expense fully. It will not affect the shared
              balance.
            </Text>
          </View>
        )}
        ...
        <TouchableOpacity style={styles.submitBtn} onPress={handleSubmit}>
          <Text style={styles.submitBtnText}>Update Expense</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  errorText: { fontSize: 16, color: "#6C6C70", textAlign: "center", marginBottom: 16 },
  backBtn: { backgroundColor: "#4A90D9", borderRadius: 12, padding: 14, paddingHorizontal: 28 },
